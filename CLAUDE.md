...
- `vercel.json` — sets `X-Robots-Tag: noindex, nofollow` on every path. With `robots.txt` (Disallow all) and a `noindex` `<meta>` in each page, this keeps the site out of search engines. It is **not** access control — anyone with the direct URL can still open it (use Vercel Deployment Protection if a real gate is needed).
